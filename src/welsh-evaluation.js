...
export function evaluateWelshCases(cases) {
  const matrix = { true_positive: 0, true_negative: 0, false_positive: 0, false_negative: 0 };
  const results = cases.map(item => {
    const actual = analyseWelsh(item.text).decision;
  ...
  return {
    profile_id: "cy-v0.1", total_cases: total, expert_approved_cases: reviewed,
    accuracy_ppm: total ? Math.floor(((matrix.true_positive + matrix.true_negative) * 1_000_000) / total) : 0,
