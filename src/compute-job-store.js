...
function equalDigest(left,right){if(typeof left!=="string"||typeof right!=="string")return false;const a=Buffer.from(left),b=Buffer.from(right);return a.length===b.length&&timingSafeEqual(a,b);}
function rowJob(row) {
  if (!row) return null;
  return {
...
export class InMemoryComputeJobStore {
  ...
  async authenticate(providerId, apiToken) { return equalDigest(this.providers.get(providerId)?.token_digest,digest(apiToken)); }
  ...
  async rotateProviderToken(providerId,apiToken) { const provider=this.providers.get(providerId);if(!provider)throw new Error("provider not found");provider.token_digest=digest(apiToken);const event={event_type:"provider_token_rotated",provider_id:providerId,reason_code:null,created_at:this.now().toISOString()};this.operationEvents.push(event);return clone(event); }
...
export class PostgresComputeJobStore {
  ...
  async rotateProviderToken(providerId,apiToken){const client=await this.pool.connect();try{await client.query("BEGIN");const now=this.now();const result=await client.query("UPDATE protocol_compute_providers SET token_digest=$1 WHERE provider_id=$2 RETURNING provider_id",[digest(apiToken),providerId]);if(!result.rowCount)throw new Error("provider not found");const event={event_type:"provider_token_rotated",provider_id:providerId,reason_code:null,created_at:now.toISOString()};await client.query("INSERT INTO protocol_compute_operations (event_type,provider_id,event_data,created_at) VALUES ($1,$2,$3::jsonb,$4)",[event.event_type,providerId,JSON.stringify(event),now]);await client.query("COMMIT");return event;}catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}}
