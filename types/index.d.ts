...
export class InMemoryComputeJobStore { constructor(options?:{now?:()=>Date}); registerProvider(input:Record<string,unknown>):Promise<Record<string,unknown>>; authenticate(providerId:string,apiToken:string):Promise<boolean>; commitment(commitmentId:string):Promise<Record<string,unknown>|undefined>; enqueue(input:Record<string,unknown>):Promise<Record<string,unknown>>; job(jobId:string):Promise<Record<string,unknown>|undefined> }
export class PostgresComputeJobStore { readonly atomicLedgerTransitions:true; constructor(options:{pool:PoolLike;now?:()=>Date}); registerProvider(input:Record<string,unknown>):Promise<Record<string,unknown>>; authenticate(providerId:string,apiToken:string):Promise<boolean>; commitment(commitmentId:string):Promise<Record<string,unknown>|null>; enqueue(input:Record<string,unknown>):Promise<Record<string,unknown>>; job(jobId:string):Promise<Record<string,unknown>|null> }
export class ComputeCoordinator { constructor(options:{registry:SettlementRegistry|PostgresSettlementRegistry;store:InMemoryComputeJobStore|PostgresComputeJobStore;now?:()=>Date;leaseMs?:number;maxAttempts?:number}); registerProvider(input:Record<string,unknown>):Promise<Record<string,unknown>>; enqueue(input:{redemptionId:string;workload:Record<string,unknown>}):Promise<Record<string,unknown>>; claim(providerId:string):Promise<Record<string,unknown>|null>; complete(providerId:string,jobId:string,input:Record<string,unknown>):Promise<Record<string,unknown>>; fail(providerId:string,jobId:string,input?:Record<string,unknown>):Promise<Record<string,unknown>>; reapExpired():Promise<Record<string,unknown>[]> }
